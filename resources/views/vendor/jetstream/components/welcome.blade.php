@php
    ...
    $orders_count = App\Models\Order::whereNull('shipped_at')
        ->get('id')
        ->count();
@endphp
...
<div class="p-6 sm:px-20 border-gray-200">
    <div class="grid grid-cols-2 gap-3">
        ...
        <a href="{{ route('orders.index') }}" class="col-span-full flex flex-col p-4 bg-white shadow-md rounded-md ">
            Órdenes
            @if ($orders_count)
                <strong class="text-red-600 inline">({{ $orders_count }} sin despachar)</strong>
            @else
                <strong class="text-green-600 inline">(Todas despachadas)</strong>
            @endif
        </a>
    </div>
</div>
